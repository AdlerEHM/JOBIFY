...
function renderizarPanelCompletar(db, postulacionId, postulacionData, proyectoData, usuarioActual,
    ...
    // Ambos completaron
    if (ambosCompletaron) {
        //pregunta si el pago esta en true 
        // para contruir bloque para mostrar el recibo 
        //MODULO 5.5 RECIBO VISUAL CONSTRUIDO EN HTML CUANDO EL PROYECTO YA ESTA EN MODO HISTORIAL 
        ...
        panel.innerHTML = `
            ...
            ${pago}`; //aqui  se finaliza todo y inyecta el html del recibo
            //el css del recibo esta en valoraiocn cssen la clase
            //recibo-pago-box
        return;
    }
                ...
                setTimeout(() => {
                    ...
                    window.paypal.Buttons({
                        ...
                        onApprove: async (data, actions) => {
                            ...
                            try {
                                //MODULO 5.2 CAMPOS DE PAGO 
                                //confirmacion proyecto completado 
                                ...
                                //notificacion interna de pago completado para el programador 
                                //MODULO 5.5 NOTIFICACION INTERNA 
                                ...
                                try {
                                    //MODULO 5.5 CONSULTA correo electronico DEL PROGRAMADOR 
                                    ...
                                    if (progSnap.exists()) {
                                        ...
                                        if (window.emailjs && progData.email) {
                                          //enviar correo al programador con los detalles del pago
                                            await emailjs.send("service_sq5han5", "template_kdb93sl", {
                                                nombre:   progData.nombre || 'Programador',
                                                to_email: progData.email, //CORREO DEL PROGRAMADOR 
                                ...
                                setTimeout(() => {
                                    //aqui se libera el proyecto completado despues de procesar el pago
                                    confirmarCompletado(db, postulacionId, postulacionData, esEmpresa, usuarioActual);
                                }, 1500);
                            } catch(e) {
    ...
    //MODULO 5.3 algoritmo para calculo de comisiones 
    // Ninguno ha confirmado
    //validacion para calculo de comision 
    if (esEmpresa) {
        //calcular comisiones 
        ...
        if (pagoRealizado) {
            ...
            //MODULO 5.1 aqui se muestra el boton de paypal 
            ...
            setTimeout(() => {
                ...
                contenedor.innerHTML = '';
                //BOTON PAYPAL RENDEREIZAR BOTON
                window.paypal.Buttons({
                    style: { layout: 'vertical', color: 'gold', shape: 'rect', label: 'pay', height: 45 },
                    createOrder: (data, actions) => actions.order.create({
                        purchase_units: [{
                    ...
                    //renderizar boton de paypal  MODULO 5.1
                }).render('#paypal-button-container');
            }, 300);
        }
    } else {
    ...
    //MODULO 5.2 ESCROW 
    //FLUJO 
    // Programador confirma que terminó
    //    ↓
    //Empresa ve PayPal — proyecto sigue activo (no completado)
    //    ↓
    //Empresa paga → PayPal ejecuta capture()
    //    ↓
    //Se guarda en Firestore: pagoRealizado: true, pagoOrderId, pagoMonto
    //    ↓
    //Solo aquí se llama confirmarCompletado() → estadoProyecto: 'completado'
    //    ↓
    //Se habilitan valoraciones y modo historial
