...
onAuthStateChanged(auth, async (user) => {
    // FIX: ignorar si estamos en flujo de Google o 2FA para evitar loop de redirecciones
    if (user && !sessionStorage.getItem('esperando2FA') && !sessionStorage.getItem('esperandoGoogle')) {
...
document.getElementById('btnGoogle').onclick = async () => {
    ...
    // FIX: bloquear onAuthStateChanged mientras manejamos el flujo de Google
    // para evitar el loop de redirecciones
    sessionStorage.setItem('esperandoGoogle', '1');
    ...
    try {
        ...
        if (!docSnap.exists()) {
            // Primera vez con Google → crear documento con el rol seleccionado
        ...
        sessionStorage.removeItem('esperandoGoogle');
        await redireccionarUsuario(user);
    } catch (error) {
        sessionStorage.removeItem('esperandoGoogle');
        mostrarError(traducirError(error.code));
